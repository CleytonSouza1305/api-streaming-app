...
module.exports = {
    ...
    try {
      ...
      if (profileName !== undefined) {
        if (profileName.length < 3) {
          throw new HttpError(
            400,
            "Formato de nome de perfil está inválido. Deve conter ao menos 3 caracteres."
          );
        }
      ...
      if (
        ...
        if (profilePin.length !== 4) {
          throw new HttpError(
            400,
            "Formato de PIN inválido. Deve conter 4 caracteres."
          );
        }
  ...
  async deleteFromListReq(req, res, next) {
    try {
      const { id, movieId } = req.params;

      const profile = await Profile.profileById(id);
      if (!profile) {
        throw new HttpError(404, `Perfil com ID ${id} não encontrado.`);
      }
      ...
      if (removeFromList.status === 404) {
        throw new HttpError(removeFromList.status, removeFromList.message);
      }
